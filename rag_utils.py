...
def save_knowledge_base(destination: str, texts: List[str], references: List[dict] = None, search_query: str = ""):
    """
    将文本保存到文件系统。
    ...
    try:
        ...
        with open(os.path.join(kb_path, 'texts.json'), 'w', encoding='utf-8') as f:
            json.dump({
                ...
                'created_at': str(os.path.getctime(kb_path) if os.path.exists(kb_path) else 0)
